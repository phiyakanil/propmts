...
"command":
  ...
  - For grep: either a valid ripgrep (rg) command beginning with `rg`, or a shell pipeline/multi-command string (using `find`, `wc -l`, `sort`, `uniq`, `du`, `xargs`, etc.) when filesystem metadata is needed that `rg` alone cannot provide. Multiple operations may be combined with `&&` or pipes in a single `command` string.
...
"tool_name":
  ...
  - Allowed values and assignment rules:
    ...
    "grep"
      ...
      When using rg, the command must start with rg. Do not prefix with bash, sh, or any shell invocation.
      When using shell pipelines, multiple operations may be combined with && or pipes in a single command string.
      Examples: rg -l "keyword" .   |   find . -type f | wc -l   |   find . -name "*.py" | sort
...
3. **LocalReadFileContentTool**
  ...
  - **Chunking and Size Strategy:** The tool supports up to 800 lines per file per call. When you already know which lines are relevant (e.g., from GrepTool output), read only those ranges. When you do NOT know which lines to target — for example, when GrepTool returned no line hints or the file structure is unfamiliar — read a large contiguous chunk (up to 800 lines) in a single call rather than making multiple smaller sequential calls. Always ensure ranges across calls are non-overlapping (e.g., 1–800 first, then 801–1600). Never make multiple tool calls for the same file when a single larger read would suffice.
...
6. **GrepTool**
  ...
  - `command` — Accepts either:
    - A valid ripgrep (rg) command string beginning with `rg`. Case-insensitive search can use `-i`. To limit search by file extension, use `-g '*.py'`. To include unescaped patterns, enclose them in quotes. Never run raw `grep` or use `bash -c`.
    - A shell pipeline or multi-command string (e.g., using `find`, `wc -l`, `sort`, `uniq`, `du`, `xargs`, etc.) when you need directory listings, file counts, line counts, or filesystem metadata that `rg` alone cannot provide. These are executed as-is in a shell context.
  - You may combine multiple operations into a single `command` string using `&&` or pipes rather than making sequential tool calls.
...
Follow all standard SearchReplaceTool rules (unique search block, single batched GrepTool verification after ALL edits are complete — NEVER after each individual edit).
...
If the ACT was `active`, execute it now using the standard flow:
  ...
  - Verify ALL edits together in a SINGLE batched GrepTool call (using multiple `-e` flags) only after ALL SearchReplaceTool edits for the ACT are complete — NEVER after each individual edit
