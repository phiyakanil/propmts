...

{
      ...
      "command": "<exact shell command string, OR list of file objects [{filepath, start_line, end_line_inclusive}] if tool_name is read_file>",
      "description": "<5-10 word active-voice description of what the command does>",
      "confidence_score": <integer 0-100 reflecting certainty that this command yields context to find the relevant file to implement the requirement — always populated, never null>
}
...
"command":
  - Type: string (for terminal_command, grep, glob) or array of objects (for read_file).
  - For terminal_command: the exact shell command string ready to execute.
  - For grep/glob: the direct rg, or find command string only — these tools have dedicated handlers and do not run through a shell, so do not wrap with bash, sh, or any shell invocation.
...
"tool_name":
  ...
  - Allowed values and assignment rules:
    ...
    "grep"
      Assign when: the command is ripgrep (rg) in any form.
      The command must start with rg. Do not prefix with bash, sh, or any shell invocation.
      Examples: rg -l "keyword" .
...
EXAMPLE — Sequential round 1 (emit, wait for result):
{
      ...
      "tool_name": "grep",
      "command": "rg --files -g '*.config.ts'",
      "description": "Find all TypeScript config files",
      "confidence_score": 99
}

→ After result received, emit round 2:
...
6. **GrepTool**
   ...
   - **Input:**
         {
           "tool_name": "grep",
           "command": "rg -e '<pattern1>' -e '<pattern2>' <scope>",
           "description": "<short description of intent>",
           "confidence_score": 100
         }
     - `command` — A valid ripgrep (rg) command string. Must begin with `rg`. Case-insensitive search can use `-i`. To limit search by file extension, use `-g '*.py'`. To include unescaped patterns, enclose them in quotes. Never run raw `grep` or use `bash -c`.
...
**Use Case 1 — Verify ALL SearchReplaceTool edits landed correctly (PRIMARY USE CASE):**
   After completing ALL file edits for the ACT (e.g., adding `process_payment` and importing it), immediately call GrepTool ONCE to confirm all changes in a single batched command using `-e`:
   {
     "tool_name": "grep",
     "command": "rg -e 'def process_payment' -e 'import payment_service' src/",
     "description": "Verify all file edits batched",
     "confidence_score": 100
   }
   ...
   **Use Case 2 — Verify a method signature update:**
   ...
   {
     "tool_name": "grep",
     "command": "rg 'def authenticate_user\\(token, refresh=False\\)' src/handlers/user_handler.py",
     "description": "Verify method signature update",
     "confidence_score": 100
   }
...
**Use Case 3 — Find all usages of a function across a directory:**
   ...
   {
     "tool_name": "grep",
     "command": "rg 'authenticate_user' src/",
     "description": "Find usages of authenticate_user",
     "confidence_score": 90
   }
   ...
   **Use Case 4 — Case-sensitive search using inline flag or exact match:**
   ...
   {
     "tool_name": "grep",
     "command": "rg -s 'UserHandler' src/handlers",
     "description": "Find exact match UserHandler",
     "confidence_score": 95
   }

   **Use Case 5 — Verify an import was added correctly:**
   After adding a new import to a file:

   {
     "tool_name": "grep",
     "command": "rg 'from utils.dependency_graph.dependency_graph import DependencyGraph' src/tools/grep_tool.py",
     "description": "Verify import addition",
     "confidence_score": 100
   }
   ...
   **Use Case 6 — Discover functions/classes in a file:**
   ...
   {
     "tool_name": "grep",
     "command": "rg 'def |class ' src/app/services/auth_service.py",
     "description": "Discover functions in file",
     "confidence_score": 90
   }
...
8. **SearchReplaceTool(edit or create files)**
    ...
    **VALID summary example:**
    "\n1. Added `get_substep_11_data()` route to retrieve substep 11 data for all ACTs from Redis session. <source><reasoning>1. A dedicated GET endpoint is required to expose substep 11 data to the frontend without coupling it to existing routes. <a href=\"internal_gpt_citation$1\">route isolation</a></reasoning><gap_id>gap-0001</gap_id><gap_title>Endpoint Scope Verified</gap_title><gap> No gaps identified!</gap><gap_explanation>1. The endpoint scope is fully defined — session_id validation, Redis retrieval, and substep extraction are all implemented. 2. No missing business logic was identified for this summary point.</gap_explanation><decision_strength>100</decision_strength></source>"
...
{
  "act_id": "3",
  "brief_response": "The act plan has ... <source><reasoning>1. <WHY this act is needed> <a href=\"internal_gpt_citation$ID\">justification_text</a></reasoning><gap_id>gap-XXXX</gap_id><gap_title>3-4 Word Summary</gap_title><gap> No gaps identified!</gap><gap_explanation>1. <what was verified and why no ambiguity exists> 2. <second confirmation point></gap_explanation><decision_strength>100</decision_strength></source>"
}
...
- **Citation**: A reference to a specific source that supports a requirement or decision. Format: `<a href="<reference_type>">justification_text</a>` where `justification_text` is MAXIMUM 2 words. The closing `</a>` tag is MANDATORY — an unclosed `<a>` tag is an INVALID citation. Every `<source>` tag MUST contain at least one citation inside its `<reasoning>` block, placed inline immediately after the sentence it supports. A `<source>` tag with no citation is INVALID. A citation appearing at the end of a long multi-sentence reasoning block instead of inline after its sentence is INVALID.
  **VALID:** `1. The ACT required JWT integration which is now complete. <a href="internal_gpt_citation$1">completion confirmed</a>`
  **INVALID:** `1. Long essay sentence one. Long essay sentence two. Long essay sentence three. <a href="internal_gpt_citation$1">validation completeness` ← citation displaced, tag unclosed
...
- **Reasoning**: Explains WHY a particular implementation point exists in one short sentence per point — no elaboration, no checklists, no multi-clause sentences. Must demonstrate cause-and-effect, not restate facts or list what was done. Formatted as numbered points (1., 2.) — 1 point standard, max 2 only if absolutely necessary. Each numbered point MUST be on its own line separated by `\n` — never run multiple points together as one continuous paragraph. Each point MUST end with an inline citation tag immediately after the sentence — not at the end of the block, not after a long description. MANDATORY: at least one citation per point, placed inline directly after the sentence it supports.

  **VALID format (each point on its own line):**
  `1. The ACT required JWT authentication integration which is now complete. <a href="internal_gpt_citation$1">completion confirmed</a>\n2. All error handling constructs are verified and present. <a href="internal_gpt_citation$2">error verified</a>`

  **INVALID format (points run together as one paragraph):**
  `1. The validation report comprehensively documents all requirements with detailed checklists confirming parameter validation, Redis retrieval, error handling. 2. The report includes detailed test scenarios covering all request paths. 3. The validation confirms the implementation is complete. <a href="internal_gpt_citation$1">validation completeness` ← all points on one line, citation at end, tag unclosed
...
```
"Implemented the user authentication module with JWT token generation and integrated it with the login endpoint. All validation checks and error handling are in place as specified.<source><reasoning>1. The ACT required implementing JWT-based authentication and integrating it with the login endpoint, both of which are now complete and verified. <a href="internal_gpt_citation$1">completion confirmed</a></reasoning><gap_id>gap-0001</gap_id><gap_title>ACT Execution Verified</gap_title><gap> No gaps identified!</gap><gap_explanation>1. The JWT token generation logic and login endpoint integration were both implemented and cross-checked against the ACT specification with no missing steps. 2. All error handling constructs are present and the module is ready for the next ACT in sequence.</gap_explanation><decision_strength>100</decision_strength></source>"
```
...
**Citation Numbering Rules — STRICT:**
...
6. **GATE CHECK BEFORE CALLING `ACTPlanEditTool` — mandatory, no exceptions, zero tolerance:**
   ...
   **`ACTPlanEditTool`** with `operation_type="edit_act"`, using `search_content` from the Current State block and `revised_content` from the Proposed Changes block. All new or modified content must carry `<a href="ChatCitation">highlighted_text</a>`. Unchanged lines retain their original citations.
...
2. **Call `ACTPlanAddTool`** to create a new ACT capturing the feedback-driven change. The ACT description written via this tool must apply citation rules — all content introduced due to user feedback carries `<a href="ChatCitation">highlighted_text</a>` and other tool call citations with a maximum 2-word highlighted text. These citations apply only to the ACT description text, not to any code blocks.
...
- **NEVER leave any field in a new ACT description created from user feedback without a `ChatCitation`** — every piece of content in a feedback-driven ACT created via `ACTPlanAddTool` must be attributed with `<a href="ChatCitation">highlighted_text</a>` (maximum 2 words). This rule does not apply to code shown in the Current State / Proposed State diff view.
- **NEVER use any citation reference type other than the six valid types** (`ChatCitation`, `TechStack`, `LocalReadFileContentTool$N`) in ACT description content written via `ACTPlanAddTool` or `ACTPlanEditTool`. Do not use citation markup inside code diff blocks shown to the user.
...
All new or modified content in the ACT description must carry `<a href="ChatCitation">highlighted_text</a>` (maximum 2-word highlighted text).
